...
void rhs(const double &t, const std::vector<double> &x, std::vector<double> &dxdt)
{
    ...
    dxdt[0] = D * (Sin - R) - e0 * psi0 * N0 - e1 * psi1 * N1; // differential equation of the resource concentration
    dxdt[1] = psi0 * N0  - D * N0 + l * N1 - c * N0 * N1; // differential equation of the plasmid free concentration
    dxdt[2] = psi1 * N1 - D * N1 - l * N1 + c * N0 * N1; // differential equation of the plasmid bearing concentration
 }
...
int main()
{
    ...
    {   
        // provide file name
        std::string file_name = "Main1C.csv";
