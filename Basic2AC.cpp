...
std::string file_name = "onlyabenmigincencon.csv";
  ...
  const double KLW1 = pow(10, -2); // attaching rate of plasmid bearing cells to wall
...
void do_analysis(std::string output_filename) 
{
  // give initial values
...
int main()
{
  try 
  {
    ...
    ofs  << "time"  << ',' << "size" << ',' << "population" << ',' << "location" << "\n";
    ofs.close();
